...
class CuteInterpreter(object):
    ...
    dic={}
    def run_arith(self, arith_node):  #산수 노드이다.
    ...
    def run_func(self, func_node):
        ...
        def InsertTable(rhs1, rhs2):
         self.dic[rhs1.value] = self.run_expr(rhs2)

        if func_node.type is TokenType.CAR:
            rhs1 = self.run_expr(rhs1)  #run_expr 은 노드에 넣는것
            if not is_quote_list(rhs1):
                print ("car error!")
            result = pop_node_from_quote_list(rhs1) #이건 하나의 노드를 quote와 연결된 것으로부터 끄내는것.
            ...
            elif is_quote_list(expr_rhs1) is not True and is_quote_list(expr_rhs2) is True:
          ...
          elif rhs1.type is TokenType.TRUE: return self.FALSE_NODE
        elif func_node.type is TokenType.COND:
            while True:
                    result = rhs1.value.next
                    cond=self.run_list(rhs1.value)
...
def Test_method(input):
    ...
    print "Calculation...." + str(print_node(result))
def Test_All():
    while True :
        result = raw_input(">>")
        if(result=="end"):
